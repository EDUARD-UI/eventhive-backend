...
public interface LocalidadRepository extends JpaRepository<Localidad, String> {
    ...
    int decrementarDisponibles(@Param("id") String id, @Param("cantidad") int cantidad);

    @Modifying
    @Query("UPDATE Localidad l SET l.disponibles = l.disponibles + :cantidad WHERE l.id = :id")
    void incrementarDisponibles(@Param("id") String id, @Param("cantidad") int cantidad);

    // Busca una localidad y trae su evento asociado
    @Query("SELECT l FROM Localidad l JOIN FETCH l.evento WHERE l.id = :id")
    java.util.Optional<com.example.demo.model.Localidad> findByIdConEvento(@Param("id") String id);
}